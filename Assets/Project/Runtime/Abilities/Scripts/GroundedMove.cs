...
public class GroundedMove : Ability
{
	...
	public override List<UnitOp> FetchUnitOps(Vector2Int targetCoord, Unit unit)
	{
		...
		for (int i = 0; i < path.Length; i++)
		{
			...
			newStepOp.playbackData.clips = new AnimationClip[1];
			newStepOp.playbackData.curves = new AnimationCurve[1];

			newStepOp.playbackData.clips[0] = runLoop;
			newStepOp.playbackData.curves[0] = runCurve;
